  ...
  try {
    ...
    for (const targetUrl of targetUrls) {
      ...
      try {
        ...
        console.warn("Auto-sync fetch warning:", targetUrl, err.message);
      }
    }

    // High-quality fallback if network was completely blocked
    if (discoveredList.length === 0) {
      for (const c of SEED_HOT_COURSES) {
        try {
          const existingCourse = await prisma.course.findUnique({ where: { slug: c.slug } });
          const item = await prisma.discoveredCourse.upsert({
            where: { url: c.url },
            update: { titleFa: c.titleFa, isHot: true },
            create: {
              url: c.url,
              slug: c.slug,
              titleFa: c.titleFa,
